...
class AmscopeCamera(CameraModule):
    ...
    def setResolution(self, resolution, mode=None):
        ...
        for i in range(0, len(self.__resolutionModes[mode])):
            if (
                self.__resolutionModes[mode][i] is resolution
                or self.__resolutionModes[mode][i] == resolution
            ):
                if mode == "stream":
                    self.liveImageGood = False
                    self.liveIndex = i
...
if __name__ == "__main__":
    ...
    def testCase(testName, expectedValues, actualValues):
        ...
        for i in range(0, len(expectedValues)):
            if (
                expectedValues[i] is not actualValues[i]
                or expectedValues[i] != actualValues[i]
            ):
                success = False
    ...
    if not openSuccess:
        print(
            testPrefixString + "Further testing requires connection to Amscope Camera"
        )
    else:
